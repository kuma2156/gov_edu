# 📘 코리아IT Java 수업

코리아IT Java 수업에 대한 내용정리를 하였습니다.

<div align="left">
    <h1>💻 학습자</h1>
    <table>
        <tr>
            <td align="center"><img src="https://github.com/user-attachments/assets/61049fd5-5e06-4b17-bb51-d925ea3e68dc" width="250"></td>
        </tr>
        <tr>
            <td align="center"><b>김명규</b></td>
        </tr>
        <tr>
            <td align="center"><b>https://github.com/kuma2156</b></td>
        </tr>
    </table>
</div>

## ☕️ 언어 (Language)
<table style="background:white; padding:10px; border-radius:8px;">
    <tr>
        <td align="center">
            <img src="https://github.com/user-attachments/assets/e345f034-71e7-4fd8-b002-5034674a2fe8" width="64" height="64">
        </td>
    </tr>
    <tr>
        <td align="center"><b>Java</b></td>
    </tr>
</table>

## ⚙️ 초기설정
#### Java
![image](https://github.com/user-attachments/assets/96ce7fef-d71e-411b-9167-bb48b5a3aa09)
#### Spring boot
![image](https://github.com/user-attachments/assets/24246438-3ad4-457b-8ce1-6e1216aabfea)
```xml
        <dependency>
            <groupId>org.springdoc</groupId>
            <artifactId>springdoc-openapi-starter-webmvc-ui</artifactId>
            <version>2.5.0</version>
        </dependency>
```

## 🗓️ 학습 일지 요약

| 날짜         | 링크                                      |
|--------------|-------------------------------------------|
| 2025-05-13   | [자바 객체지향 핵심 개념 정리 (static, 인터페이스, 싱글톤, 업캐스팅)](src/main/java/_memo/2025-05-13_memo.md)         |
| 2025-05-14   |  [자바(제네릭, 람다, 추상화 vs 인터페이스, 컬렉션, 와일드카드, 익명클래스)](src/main/java/_memo/2025-05-14_memo.md)                                         |
| 2025-05-15   |   [자바(컬렉션 예제문제 풀기)](src/main/java/_memo/2025-05-15_memo.md)                                       |
